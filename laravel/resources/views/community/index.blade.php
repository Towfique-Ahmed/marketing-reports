...
@section('topbar-actions')
    <a href="{{ route('community.export', ['month'=>$month,'year'=>$year]) }}" class="btn btn-sm btn-outline-secondary">
    ...
    <div class="card-body py-2">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
            ...
            <div>
                ...
                <div class="text-muted" style="font-size:0.72rem;">TOTAL POSTS</div>
            </div>
        </div>
    </div>
</div>
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($post->url)
                                <a href="{{ $post->url }}" target="_blank" class="text-primary small">
                                    {{ Str::limit($post->url, 40) }} <i class="bi bi-box-arrow-up-right" style="font-size:.7rem;"></i>
                                </a>
                            @else <span class="text-muted">—</span> @endif
                        </td>
                        <td class="text-muted small">
                            @if($post->month && $post->year)
                                {{ date('M', mktime(0,0,0,$post->month,1)) }} {{ $post->year }}
                            @else —
                            @endif
                        </td>
                    ...
                    <tr>
                        <td colspan="6" class="text-center py-5 text-muted">
                            <i class="bi bi-inbox fs-2 d-block mb-2"></i>No community posts found.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
<div class="modal fade" id="importModal" tabindex="-1">
    <div class="modal-dialog"><div class="modal-content">
        ...
        <form action="{{ route('community.import') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="modal-body">
                <p class="text-muted small">Columns: <code>id, title, post_date, month, year, url, notes</code></p>
