...
@section('topbar-actions')
    <a href="{{ route('documentations.export', ['month'=>$month,'year'=>$year]) }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-download me-1"></i>Export CSV</a>
    ...
    <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#addModal"><i class="bi bi-plus-lg me-1"></i>Add Doc</button>
@endsection

@section('content')
<div class="card mb-3"><div class="card-body py-2"><div class="d-flex flex-wrap align-items-center justify-content-between gap-2"><x-month-year-filter /><div><div class="fw-bold text-primary">{{ number_format($total) }}</div><div class="text-muted" style="font-size:0.72rem;">TOTAL DOCS</div></div></div></div></div>
...
<div class="card"><div class="card-body p-0"><div class="table-responsive"><table class="table table-hover align-middle mb-0">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>@if($doc->url)<a href="{{ $doc->url }}" target="_blank" class="text-primary small">{{ Str::limit($doc->url, 40) }} <i class="bi bi-box-arrow-up-right" style="font-size:.7rem;"></i></a>@else<span class="text-muted small">—</span>@endif</td>
            <td class="text-muted small">@if($doc->month && $doc->year){{ date('M', mktime(0,0,0,$doc->month,1)) }} {{ $doc->year }}@else —@endif</td>
            <td><span class="text-muted small">{{ Str::limit($doc->notes, 50) ?: '—' }}</span></td>
            <td class="text-end text-nowrap">
                <button class="btn btn-sm btn-outline-primary btn-edit" data-id="{{ $doc->id }}" data-title="{{ $doc->title }}" data-url="{{ $doc->url }}" data-publish_date="{{ $doc->publish_date }}" data-month="{{ $doc->month }}" data-year="{{ $doc->year }}" data-notes="{{ $doc->notes }}" data-bs-toggle="modal" data-bs-target="#editModal"><i class="bi bi-pencil"></i></button>
                <button class="btn btn-sm btn-outline-danger btn-delete" data-id="{{ $doc->id }}" data-title="{{ $doc->title }}" data-bs-toggle="modal" data-bs-target="#deleteModal"><i class="bi bi-trash"></i></button>
            </td>
        </tr>
        @empty
        <tr><td colspan="6" class="text-center py-5 text-muted"><i class="bi bi-inbox fs-2 d-block mb-2"></i>No documentation found.</td></tr>
        @endforelse
    </tbody>
</table></div></div>@if($docs->hasPages())<div class="card-footer bg-white py-2">{{ $docs->links() }}</div>@endif</div>

<!-- Add Modal -->
<div class="modal fade" id="addModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <div class="modal-header"><h5 class="modal-title">Add Documentation</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
    <form action="{{ route('documentations.store') }}" method="POST">@csrf
...
<div class="modal fade" id="editModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <div class="modal-header"><h5 class="modal-title">Edit Documentation</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
...
<div class="modal fade" id="deleteModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <div class="modal-header"><h5 class="modal-title text-danger">Delete Documentation</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
...
<div class="modal fade" id="importModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    ...
    <form action="{{ route('documentations.import') }}" method="POST" enctype="multipart/form-data">@csrf
        <div class="modal-body"><p class="text-muted small">Columns: <code>id, title, url, publish_date, month, year, notes</code></p><input type="file" name="csv_file" class="form-control" accept=".csv,.txt" required></div>
